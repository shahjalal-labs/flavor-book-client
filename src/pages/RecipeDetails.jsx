...
const RecipeDetails = ({ currentUser }) => {
  ...
  if (!recipe) {
    return (
      <div className="text-center py-10 text-indigo-400 animate-pulse">
        Loading...
      </div>
    );
  }
  ...
  return (
    <>
      <h2 className="text-xl sm:text-2xl font-semibold mb-8 text-center text-indigo-400 mt-10">
        <span>{likeCount || "No"}</span> Foodies love this recipe.
      </h2>

      <div className="max--4xl mx-auto p-6 rounded-xl shadow-2xl bg-gradient-to-br from-slate-900 via-gray-900 to-gray-950 border border-indigo-700/30">
        <Helmet>
          <title>Recipe Details | FlavorBook</title>
        </Helmet>

        <h1 className="capitalize text-2xl font-bold mb-4 font-kaushan text-indigo-400">
          {recipe.title}
        </h1>
        ...
        <div
          className={`${
            recipe.image && isImageValid
              ? "sm:w-[450px]"
              : "sm:w-[450px] h-[300px] border bg-gray-800 flex items-center justify-center"
          } rounded-xl mb-4 shadow-lg border border-indigo-700`}
        >
          <img
            src={
              recipe.image && isImageValid
        ...
        <p className="text-lg mb-2 text-indigo-300">
          <strong>Cuisine Type:</strong> {recipe.cuisine}
        </p>
        ...
        <div className="mb-4">
          <strong className="text-indigo-300">Ingredients:</strong>
          <ul className="p-2 rounded mt-1 space-y-1 bg-gray-800 text-slate-300">
            {recipe.ingredients.map((ingredient, index) => (
              <li key={index}>
        ...
        <div className="mb-4">
          <strong className="text-indigo-300">Instructions:</strong>
          <p className="bg-gray-800 text-slate-300 p-2 rounded mt-1">
            {recipe.instructions}
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <button
            onClick={handleLike}
            className="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 transition-shadow shadow-lg shadow-indigo-500/40"
          >
            ❤️ Like
          </button>
        </div>
